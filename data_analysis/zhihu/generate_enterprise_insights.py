...
def compute_weight(df: pd.DataFrame) -> pd.Series:
    vote = pd.to_numeric(df.get("voteup_count"), errors="coerce").fillna(0.0).clip(lower=0.0)
    com = pd.to_numeric(df.get("comment_count"), errors="coerce").fillna(0.0).clip(lower=0.0)
    return 1.0 + np.log1p(vote) + 0.2 * np.log1p(com)
...
def build_config(base_dir: str) -> PlatformConfig:
    return PlatformConfig(
        name="zhihu",
        id_col="answer_id",
        prediction_path=os.path.join(base_dir, "model_prediction", "zhihu", "prediction", "roBERTa_prediction.csv"),
        bertopic_dir=os.path.join(base_dir, "model_prediction", "zhihu", "prediction", "bertopic_output"),
...
def generate(out_dir: str, base_dir: str, top_n_topics: int, top_n_demands: int):
    ...
    safe_mkdir(out_dir)
    topic_out_path = os.path.join(out_dir, "enterprise_negative_topics_zhihu.csv")
    demand_out_path = os.path.join(out_dir, "enterprise_negative_demands_zhihu.csv")
    trend_out_path = os.path.join(out_dir, "enterprise_negative_demand_trend_daily_zhihu.csv")
